...
caste_plotter <- function(caste){
        ...
        raw_count_directory = paste("input/count_estimates_", caste, "/", sep="")
        load(paste(raw_count_directory, "txi_count_estimates.Rdata", sep=""))
        ## Load significant genes for plotting:
        load(paste("input/", pesticide, "_de_genes_output_", caste, "/significant_genes.Rdata", sep=""))
        ...
        ## Using custom script, load gene annotations from NCBI for each significant gene:
        gene_descriptions <- description_from_LOCid(de_genes_CLO)
        
        ##
        pesticide_abbrev <- toupper(substr(pesticide, start = 1, stop = 3))
        control_abbrev <- toupper(substr(control, start = 1, stop = 3))
        ...
        ## 
        column_nums <- c(grep(pesticide_abbrev, colnames(normalised_counts_df_sig_df)), 
                         grep(control_abbrev, colnames(normalised_counts_df_sig_df)))
        
        ## Subset columns of interest:
        normalised_counts_df_sig_subset <- normalised_counts_df_sig_df[, column_nums]
        
        ## Rename columns:
        colnames(normalised_counts_df_sig_subset) <- sample_names
        
        ## Add rownames with gene descriptions:
        rownames(normalised_counts_df_sig_subset) <- gene_descriptions$id
        normalised_counts_df_sig_subset$description <- gsub(":", "", gene_descriptions$both)
        ...
        dd<-hclust(dist(as.matrix(normalised_counts_df_sig_subset[,1:8])))

        ## Reorder based on cluster order: 
        normalised_counts_df_sig_subset_ordered <- as.data.frame(normalised_counts_df_sig_subset[dd$order, ])

        ## Reshape ("melt") for plotting with ggplot2:
        normalised_counts_df_sig_subset_ordered_melt <- melt(normalised_counts_df_sig_subset_ordered)

        ## Rename columns for plotting:
        colnames(normalised_counts_df_sig_subset_ordered_melt) <- c("gene_name", "sample", "normalised_counts")
        ...
        normalised_counts_df_sig_subset_ordered_melt$gene_name <- 
        factor(normalised_counts_df_sig_subset_ordered_melt$gene_name, levels = rev(unique(c(as.character(unlist(normalised_counts_df_sig_subset_ordered_melt$gene_name))))))
        
        ## Create a treatment column to plot:
        normalised_counts_df_sig_subset_ordered_melt$treatment <- rep(c(pesticide, control), 
                                                              each = (nrow(normalised_counts_df_sig_subset_ordered_melt)/2))
        
        ## Depending on which caste, call a specific function to plot:
        if (caste=="workers"){
                plot <- heatmap_plot_worker(normalised_counts_df_sig_subset_ordered_melt)
                return(plot)
        } else {
                plot <- heatmap_plot_queen(normalised_counts_df_sig_subset_ordered_melt)
                return(plot)
        }
