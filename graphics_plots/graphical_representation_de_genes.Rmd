...
new_sample_names <- c(1:4,5:8,9:12)
...
significant_de_counts_df_reordered_melt$treatment <- rep(c("Clothianidin", "Control", "Imidacloprid"), each = 44)
...
heatmap_plot_queens <- ggplot(significant_de_counts_df_reordered_melt, aes(sample, gene_name)) +
                ...
                scale_y_discrete(position = "right") +
                labs(fill = 'Log fold change')  +
                geom_text(x = 2.5, y = -0.35, label = "Clothianidin", size = 7, fontface = "bold") +
                geom_text(x = 6.5, y = -0.35, label = "Control", size = 7, fontface = "bold") +
                geom_text(x = 10.5, y = -0.35, label = "Imidacloprid", size = 7, fontface = "bold") 
