...
de_genes_file <- "input/input_for_go_term_analysis/wald_clo/de_genes_for_topGO.txt"

output_directory <- "results/output_node_100_clo"
...
for (go_category in c("BP", "MF", "CC")) {
  ...
  write.table(x         = result_weight_output,
              file      = file.path(output_directory,
                                    paste(go_category, "raw.txt", sep = "_")),
              row.names = FALSE,
              quote = FALSE,
              sep       = "\t")
  ...
  ## Update column names:
  colnames(result_weight_output_sig)[6] <- gsub(" ", "_", colnames(result_weight_output_sig)[6])
  
  ## For significant terms, add an additional field called 'category' which will be used for plotting of individual go category:
  ...
  write.table(x         = result_weight_output_sig,
              file      = file.path(output_directory,
                                    paste(go_category, "sig.txt", sep = "_")),
              row.names = FALSE,
              quote = FALSE,
              sep       = "\t")
}
