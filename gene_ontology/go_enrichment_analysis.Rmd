...
my_go_data <- new(object="topGOdata", 
...
result_weight_output <- GenTable(object=myGOdata, 
                                 weight_ks = result_weight_ks, 
                                 weight_fisher = result_weight_fisher,
                                 orderBy = "weight", 
                                 topNodes = length(score(weight_ks)))

## Subset calls with significance higher than expected:
result_weight_output_sig <- subset(combined_results, Significant > Expected &
                                   weight_ks < 0.05)
...
print(showGroupDensity(my_go_data, 
                       head(result_weight_output_sig$, n=1), 
                       ranks = TRUE, 
                       rm.one = FALSE))
