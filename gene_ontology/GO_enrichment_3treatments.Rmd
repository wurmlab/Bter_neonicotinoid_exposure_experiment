...
```{r}
# Set input files:
genes_of_interest_3_treatments <- read.table("de_genes_for_topGO_LRT.txt", header = T)
...
# Define a cut-off:
cutoff_for_top_fivepercent_LRT <- quantile(x = geneList_3_treatments, probs = 0.05)
...
```{r}
# Set input files:
genes_of_interest_pairwise_CON_CLO <- read.table("de_genes_for_topGO_pairwise_CLO.txt", header = T)
...
# Define a cut-off:
cutoff_for_top_fivepercent_pairwise <- quantile(x = geneList_pairwise_CON_CLO, probs = 0.05)
...
myGOdata_pairwise_CON_CLO <- new("topGOdata", 
                ...
                ontology = GO_term_type, 
                #geneSel = function(x) {return(x=TRUE)}, # Run just for ks test
                geneSel = function(x) {return(x < cutoff_for_top_fivepercent_pairwise)}, # only used for fisher test
...
## Write to output:
write.table(allRes_3_treatments, file = "allRes_3_treatments_GO_results.txt", row.names = TRUE, sep = "\t")
...
allRes_pairwise_CON_CLO <- GenTable(myGOdata_pairwise_CON_CLO, 
                   kselim = resultElimKs_pairwise_CON_CLO, 
...
## Write to output:
write.table(allRes_pairwise_CON_CLO, file = "allRes_pairwise_CON_CLO_GO_results.txt", row.names = TRUE, sep = "\t")
