...
## Step One
## GO annotations
gene_to_go_mapping_file <- "./bter_renamed_drosophila_go_terms.input_for_topgo.txt"

## file of significant genes (2 column file: i.e. gene id and pvalue) file:
de_genes_file <- "./2018-02-08_tmp_file_filtered_sorted-onlyscored.txt"

output_directory <- "output_10_new"
...
## Read in input file:
de_genes <- read.table(de_genes_file, header = FALSE)

colnames(de_genes) <- c("locus", "adjusted_pvalue")

## Sort by adjusted p-value:
de_genes_sorted <- de_genes[order(de_genes$adjusted_pvalue), ]
...
topgo_genelist        <- de_genes_sorted_filtered$adjusted_pvalue
...
for (go_category in c("BP", "MF", "CC")) {
  ...
  result_weight_output_sig$weight_fisher_adjusted <- p.adjust(p=result_weight_output_sig$weight_fisher,
                                                          method=c("bonferroni"),
                                                          n=length(result_weight_output_sig$weight_fisher))
